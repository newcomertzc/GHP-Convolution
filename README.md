...
| `--backbone-func`                | `resnet50, vgg13, convnext_tiny (function in torchvision.models or core.convnext)` |
...
| `--replace-stride-with-dilation` | `adjust the dilation of ResNet. \[False, True, True\] indicates XXX-ResNet-8s`     |
| `--use-deterministic-algorithms` | `if specified, use deterministic algorithms. Otherwise, use faster algorithms`     |
| `--test-only`                    | `if specified, only test the network`                                              |
...
```
### FCN-ResNet-8s (DeepLabV3-ResNet-8s)
```
python train_segmentation.py --pretrained your_pretrained_checkpoint --backbone FCN_ResNet
```
