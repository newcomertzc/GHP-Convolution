...
def main(args):
    ...
    # get transform
    toJPEG = get_JPEG_transform(args.data_stat)
    valid_input_type = {'green', 'gray', 'green'}
    if args.input_type.lower() not in valid_input_type:
        raise ValueError(f"Invalid input_type {args.input_type}. Only \"green\", \"gray\" and"
                         f"\"rgb\" are supported.")
    else:
        input_transform = get_input_transform(args.input_type.lower())
...
def get_JPEG_transform(stat_path: str) -> RandomJPEGCompress:
    ...
    return RandomJPEGCompress(quality_values, quality_probs, subsampling_probs)
...
def show_model(model: nn.Module, input_size: tuple) -> None:
    ...
    for iter_num, data in tqdm(enumerate(dataloader)):
        ...
        if (iter_num + 1) % print_freq == 0:
            print(header + 
                  ...
                  f" acc: {metrics.calc_accuracy(cmatrix)*100:05.2f} "
...
def evaluate(
    ...
    print(f" acc: {metrics.calc_accuracy(metric_keeper.val_cmatrix)*100:05.2f} "
          f" loss: {metric_keeper.running_val_loss[0] / metric_keeper.running_val_loss[1]:.4f} ")
            

def print_inconsistent_kwargs(kwargs_cur: dict, kwargs_prev: dict) -> None:
...
def get_args_parser(add_help=True):
    ...
    # )
    parser.add_argument('--show-model', default=False, action='store_true', help='show model information')
